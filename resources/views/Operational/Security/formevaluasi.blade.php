...
@if (isset($securityticket->evaluasi_form))
    @php
        $evaluasiform = $securityticket->evaluasi_form;
        ...
        $rate_a1 = array_sum($array_a1) / count($array_a1); 
        $rate_a2 = array_sum($array_a2) / count($array_a2); 
        $rate_a3 = array_sum($array_a3) / count($array_a3); 

        $total_a1 = $rate_a1 / 5 * 30;
        $total_a2 = $rate_a2 / 5 * 20;
        $total_a3 = $rate_a3 / 5 * 10;
            ...
            <table class="table table-bordered text-sm">
                ...
                <tbody>
                    <tr>
                        ...
                        <td rowspan="5" id="rate_a1">{{ $rate_a1 }}</td>
                        <td rowspan="5">30%</td>
                        <td rowspan="5" id="total_a1">{{ $total_a1 }}%</td>
                        <td>&nbsp;</td>
                        @for ($i = 0; $i < 5; $i++) 
                            <td>
                                @php
                    ...
                    <tr style="line-height: 30px">
                        ...
                        <td rowspan="10" id="rate_a2">{{ $rate_a2 }}</td>
                        <td rowspan="10">20%</td>
                        <td rowspan="10" id="total_a2">{{ $total_a2 }}%</td>
                        <td>&nbsp;</td>
                        @for ($i = 0; $i < 5; $i++) 
                            <td>
                                @php
                    ...
                    <tr style="line-height: 30px">
                        ...
                        <td rowspan="2" id="rate_a3">{{ $rate_a3 }}</td>
                        <td rowspan="2">10%</td>
                        <td rowspan="2" id="total_a3">{{ $total_a3 }}%</td>
                        <td>&nbsp;</td>
                        @for ($i = 0; $i < 5; $i++) 
                            <td>
                                @php
                    ...
                    <tr>
                        ...
                        <td class="bg-success text-light text-right" colspan="3" id="subtotal_value">{{ $total_a1 + $total_a2 + $total_a3 }}%</td>
                        <td>&nbsp;</td>
        ...
        <div class="row">
                ...
                <table class="table table-sm table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="bobot_nilai_lembaga" data-value="{{ $list['bobot'] }}">{{ $list['bobot'] }}%</td>
                            <td class="total_nilai_lembaga">
                        ...
                        <tr>
                            ...
                            <td class="text-light bg-success" colspan="3" id="subtotalb_value">{{ $total_lembaga }}%</td>
                            <td>&nbsp;</td>
            ...
            <div class="col-12 border p-3 mt-2 border-dark mx-2 d-flex justify-content-start align-items-center">
                ...
                <div class="row ml-5">
                    <div class="col-12">
                        <i class="far 
                            @if ($evaluasiform->kesimpulan == 0)
                                fa-check-square
                            @else
                                fa-square
                            @endif 
                            mr-2" aria-hidden="true"></i><span>VENDOR DAN PERSONIL TETAP</span>
                    </div>
                    <div class="col-12">
                        <i class="far 
                            @if ($evaluasiform->kesimpulan == 1)
                                fa-check-square
                            @else
                                fa-square
                            @endif 
                             mr-2" aria-hidden="true"></i><span>GANTI VENDOR</span>
                    </div>
                    <div class="col-12">
                        <i class="far 
                            @if ($evaluasiform->kesimpulan == 2)
                                fa-check-square
                            @else
                                fa-square
                            @endif 
                            mr-2" aria-hidden="true"></i><span>GANTI PERSONIL SECURITY DENGAN VENDOR SAMA</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
                        ...
                        <tr>
                            @foreach($evaluasiform->authorizations as $authorization)
                                <td width="{{ 100/$count }}%" class="align-bottom small" style="height: 80px">
                                    @if (($evaluasiform->current_authorization()->employee_id ?? -1) == $authorization->employee_id)
            ...
            <div class="col-12 text-center">
                @if (($evaluasiform->current_authorization()->employee_id ?? '-1') == Auth::user()->id)
                    <button type="button" class="btn btn-success" onclick="evaluasiapprove({{ $evaluasiform->id }})">Approve</button>
                    <button type="button" class="btn btn-danger" onclick="evaluasireject({{ $evaluasiform->id }})">Reject</button>
                @endif
            </div>
        </div>
    </div>
@else
    <div class="d-flex flex-column box p-3">
        <b>NAMA VENDOR : {{ $securityticket->po_reference->security_ticket->vendor_name }}</b>
        <b>PERIODE PENILAIAN : {{ now()->translatedFormat('F Y') }}</b>
        <b>CABANG/DEPO : {{ $securityticket->po_reference->security_ticket->salespoint->name }}</b>
        <b class="mt-3">A. ASPEK PENILAIAN PERSONIL SECURITY</b>
        <div>
            <table class="table table-bordered text-sm">
                <thead>
                    ...
                    <tr class="bg-success">
                        <td><input type="text" class="nama_personil form-control form-control-sm" data-col="0"
                                placeholder="Nama" onchange="setColumn(this,0)"></td>
                        <td><input type="text" class="nama_personil form-control form-control-sm" data-col="1"
                                placeholder="Nama" onchange="setColumn(this,1)"></td>
                        <td><input type="text" class="nama_personil form-control form-control-sm" data-col="2"
                                placeholder="Nama" onchange="setColumn(this,2)"></td>
                        <td><input type="text" class="nama_personil form-control form-control-sm" data-col="3"
                                placeholder="Nama" onchange="setColumn(this,3)"></td>
                        <td><input type="text" class="nama_personil form-control form-control-sm" data-col="4"
                                placeholder="Nama" onchange="setColumn(this,4)"></td>
                    </tr>
                </thead>
                @php
                $row_count = 0;
                ...
                <tbody>
                    <tr>
                        ...
                        <td>&nbsp;</td>
                        @for ($i = 0; $i < 5; $i++) <td><input type="number"
                                class="nilai_personil form-control form-control-sm autonumber" data-row="{{ $row_count }}"
                                data-col="{{ $i }}" placeholder="-" min="0" max="5" disabled min="0" max="5" disabled></td>
                            @endfor
                            @php $row_count++; @endphp
                    </tr>
                    @php
                    ...
                    <tr style="line-height: 30px">
                        ...
                        @for ($i = 0; $i < 5; $i++) <td><input type="number"
                                ...
                                data-row="{{ $row_count }}" data-col="{{ $i }}" min="0" max="5" disabled min="0" max="5"
                                disabled></td>
                            @endfor
                            @php $row_count++; @endphp
                    </tr>
                    ...
                    <tr style="line-height: 30px">
                        ...
                        @for ($i = 0; $i < 5; $i++) <td><input type="number"
                                class="nilai_personil form-control form-control-sm autonumber" placeholder="-"
                                data-row="{{ $row_count }}" data-col="{{ $i }}" min="0" max="5" disabled></td>
                            @endfor
                            @php $row_count++; @endphp
                    </tr>
                ...
                <table class="table table-sm table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>&nbsp;</td>
                            <td><textarea class="form-control form-control-sm keterangan" rows="3"></textarea></td>
                        </tr>
                        @endforeach
                        <tr>
                            <td class="text-light bg-success" colspan="3">SUB TOTAL</td>
        ...
        <div class="row">
            ...
            <div class="col-12 text-center">
                <button type="button" onclick="doSubmit()" class="btn btn-primary">Submit Form dan Mulai Otorisasi</button>
            </div>
        </div>
    </div>
@endif
...
<script>
    ...
    function calculate_rate(el) {
        ...
        if (row <= 4) {
            ...
            let total_percentage = parseInt(average / 5 * 30);
            if (!isNaN(average)) {
            ...
            let average = calculateAverage(values).toFixed(2);
            let total_percentage = parseInt(average / 5 * 20);
            if (!isNaN(average)) {
            ...
            let average = calculateAverage(values).toFixed(2);
            let total_percentage = parseInt(average / 5 * 10);
            if (!isNaN(average)) {
