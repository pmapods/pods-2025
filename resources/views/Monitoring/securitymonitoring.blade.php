...
<div class="content-header">
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark">Monitoring Security</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item">Monitoring</li>
                    <li class="breadcrumb-item active">Monitoring Security</li>
                </ol>
            </div>
        </div>
    </div>
            ...
            <table id="monitoringPO" class="table table-bordered dataTable" role="grid">
                ...
                <tbody>
                    ...
                    @foreach ($pos as $po)
                        <tr>
                            <td>{{ $po->no_po_sap }}</td>
                            <td>{{ $po->security_ticket->type() }}</td>
                            <td>{{ $po->updated_at->translatedFormat('d F Y')}}</td>
                            <td>{{ $po->security_ticket->vendor_name }}</td>
                            <td>{{ $po->security_ticket->salespoint->name }}</td>
                        </tr>
                    @endforeach
                    @foreach ($end_kontrak_tickets as $ticket)
                        ...
                        <tr class="table-danger">
                            ...
                            <td>{{ $po->security_ticket->vendor_name }}</td>
                            <td>{{ $po->security_ticket->salespoint->name }}</td>
                            <td>{{ $po->security_ticket->security->plate }}</td>
                            <td>{{ $po->security_ticket->security_type->brand_name }} {{ $po->security_ticket->security_type->name }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
