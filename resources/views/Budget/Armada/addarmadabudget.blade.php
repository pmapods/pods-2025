        ...
        <div class="col-6 d-flex align-items-center">
            <div class="form-group mr-2">
              ...
              <input type="file" class="form-control-file" 
                placeholder="Pilih File Template Armada" id="file_template"
                accept=".xls, .xlsx"/>
            </div>
            <div>
            ...
            <div>
                <a class="btn btn-info mr-2" href="/template/armada_budget_template.xlsx">Download Template</a>
            </div>
        </div>
    ...
    <script>
        ...
        function listToTable() {
            ...
            let filtered_data = fileobject.filter(function(object){
                if(object["TYPE ARMADA"] != null && object["VENDOR"] != null && object["QTY"] != null && object["VALUE"] != null && object["AMOUNT"] !=null){
                    return true;
                }else{
                    return false;
                }
            ...
            filtered_data.forEach(function(data){
                $append_row = '<tr>';
                $append_row += '<td>'+data["TYPE ARMADA"]+'</td>';
                $append_row += '<td>'+data["VENDOR"]+'</td>';
        ...
        function submitBudget(){
            ...
            $('#template_table tbody tr').each(function(index){
                count++;
                let type_armada = $(this).find('td:eq(0)').text().trim();
                let vendor = $(this).find('td:eq(1)').text().trim();
                let qty = $(this).find('td:eq(2)').text().trim();
                let value = $(this).find('td:eq(3)').text().trim();
                let amount = $(this).find('td:eq(4)').text().trim();
                append_input_text += "<input type='hidden' name='item["+index+"][type_armada]' value='"+type_armada+"'>";
                append_input_text += "<input type='hidden' name='item["+index+"][vendor]' value='"+vendor+"'>";
                    ...
                    success: function (response) {
                        ...
                        if(data.budget != null){
                            ...
                            data.lists.forEach(function(item,index){
                                ...
                                append_row_text += '<td>'+item.type_armada+'</td>';
                                append_row_text += '<td>'+item.vendor+'</td>';
