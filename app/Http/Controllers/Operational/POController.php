...
class POController extends Controller
{
    ...
    public function uploadInternalSignedFile(Request $request)
    {
        ...
        try {
            ...
            $name = $po->no_po_sap . "_INTERNAL_SIGNED_" . $ticket->code . '.' . $ext;
            $path = "/attachments/ticketing/" . $type . "/" . $ticket->code . '/po/' . $name;
            $file = pathinfo($path);
            // dd($path, $file, Storage::disk('public')->exists($path));
            if (Storage::disk('public')->exists($path)) {
                // Storage::delete($path);
                Storage::disk('public')->delete($path);
            }
            // dd($file, $path, Storage::disk('public')->delete($path));
            ...
            if (!$request->file('additional_po_file')) {
                ...
                $name = $po->no_po_sap . "_ADDITIONAL_FILE_PO_" . $ticket->code . '.' . $ext;
                $path = "/attachments/ticketing/" . $type . "/" . $ticket->code . '/po/' . $name;
                $file = pathinfo($path);
