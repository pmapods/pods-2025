        ...
        try {
            if ($request->type == 'armada') {
                ...
                if ($request->block_type == 'block_day') {
                    ...
                    if (now()->day > $ticketing_block->max_mutasi_block_day) {
                        throw new \Exception('Mutasi Armada maksimal tanggal ' . $ticketing_block->max_mutasi_block_day . ' di bulan berjalan');
                    }
                    // check apakah tanggal sebelum atau setelah tanggal block day
                    if (now()->day > $ticketing_block->mutasi_block_day) {
                        $ticketing_block_open_request = TicketingBlockOpenRequest::where('status', 1)->where('ticket_code', $request->ticket_code)->first();
                        if ($ticketing_block_open_request == null) {
                            throw new \Exception('Mutasi Armada hanya dapat dilakukan pada tanggal 1-' . $ticketing_block->mutasi_block_day . ' setiap bulannya');
                        }
                    }
                    return response()->json([
                        'error' => false,
