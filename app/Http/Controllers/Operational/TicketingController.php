...
class TicketingController extends Controller
{
    public function ticketingView(Request $request)
    {
        ...
        return view('Operational.ticketing', compact('available_salespoints', 'indirect_salespoints'));
    }

    public function ticketingData(Request $request)
    {
        ...
        if ($request->type == "ticket") {
            ...
            $budget_type_array = [];
            if (str_contains(strtolower("Budget"), strtolower($search_value))) {
                array_push($budget_type_array, 0);
            }
            if (str_contains(strtolower("Non Budget"), strtolower($search_value))) {
                array_push($budget_type_array, 1);
            }
            ...
            $ticketing =  Ticket::leftJoin('salespoint', 'salespoint.id', '=', 'ticket.salespoint_id')
                ...
                ->where(function ($query) use ($employee_access, $search_value) {
                    ...
                    $query->whereIn('ticket.salespoint_id', $employee_access)
                        ->orwhere('ticket_authorization.employee_id', Auth::user()->id);
                })
                ->where(function ($query) use ($search_value, $item_type_array, $request_type_array, $budget_type_array) {
                    // filter apakan punya akses
                    $query->where(DB::raw('lower(ticket.code)'), 'like', '%' . strtolower($search_value) . '%')
                        ...
                        ->orwhere(DB::raw('lower(ticket_item.name)'), 'like', '%' . strtolower($search_value) . '%')
                        ->orwhere(DB::raw('lower(ticket.division)'), 'like', '%' . strtolower($search_value) . '%')
                        ->orWhereIn('ticket.item_type', $item_type_array)
                        ->orWhereIn('ticket.request_type', $request_type_array)
                        ->orWhereIn('ticket.budget_type', $budget_type_array);
                })
            ...
            if ($request->searchBySalesPoint) {
                $ticketing = $ticketing->where('salespoint_id', '=', $request->searchBySalesPoint);
            }
            if ($request->searchByMonth) {
                $ticketing = $ticketing->where(DB::raw('month(ticket.created_at)'), '=', $request->searchByMonth);
            }
            if ($request->searchByyear) {
                $ticketing = $ticketing->where(DB::raw('year(ticket.created_at)'), '=', $request->searchByyear);
            }
            ...
            foreach ($tickets_paginate as $ticket) {
                ...
                if (isset($ticket->created_by)) {
                    $created_by_employee = $ticket->created_by_employee->name;
                }
                $keterangan = "";
                if (isset($ticket->item_type)) {
                    $keterangan .= "Jenis Item : " . $ticket->item_type();
                }
                if (isset($ticket->request_type)) {
                    $keterangan .= "\n";
                    $keterangan .= "Jenis Permintaan : " . $ticket->request_type();
                }
                if (isset($ticket->budget_type)) {
                    $keterangan .= "\n";
                    $keterangan .= "Jenis Budget : " . $ticket->budget_type();
                }
                if (isset($ticket->is_it)) {
                    $keterangan .= "\n";
                    $keterangan .= "Jenis IT : " . (($ticket->is_it) ? "IT" : "Non IT");
                }
                if (isset($ticket->division)) {
                    $keterangan .= "\n";
                    $keterangan .= "Divisi : " . $ticket->division;
                }
                $extra_text = "";

                foreach ($ticket->ticket_item as $titem) {
                    foreach ($titem->ticket_item_attachment->where('status', -1) as $key => $item) {
                        $extra_text .= "\n<span class='text-danger'>" . $item->name . ' : ' . $item->reject_notes . "</span>";
                    }
                    foreach ($titem->ticket_item_file_requirement->where('status', -1) as $key => $item) {
                        $extra_text .= "\n<span class='text-danger'>" . $item->file_completement->name . ' : ' . $item->reject_notes . "</span>";
                    }
                }

                // trim with max 200 character
                $status_text = $ticket->status("complete") . $extra_text;
                if (strlen($status_text) > 200) {
                    $status_text = substr($status_text, 0, 200) . '...';
                }

                $rejected_file_requirement = 0;
                $employee_terminateddd = "";

                foreach ($ticket->ticket_item as $titem) {
                    $rejected_file_requirement += $titem->ticket_item_attachment->where('status', -1)->count();
                    $rejected_file_requirement += $titem->ticket_item_file_requirement->where('status', -1)->count();
                ...
                if ($rejected_file_requirement > 0) {
                    $status_text = "Otorisasi Pengadaan Selesai / di reject oleh : " . $employee_terminateddd . $extra_text;
                }
                ...
                array_push($array, Carbon::parse($ticket->requirement_date)->translatedFormat('d F Y'));
                array_push($array, implode(",\n", array_values($ticket->ticket_item->pluck('name')->toArray())));
        ...
        if ($request->type == "armada") {
            ...
            $armada_ticketing =  ArmadaTicket::leftJoin('salespoint', 'salespoint.id', '=', 'armada_ticket.salespoint_id')
                ...
                ->where(function ($query) use ($employee_access, $search_value) {
                    // filter apakah punya akses
                    $query->whereIn('armada_ticket.salespoint_id', $employee_access)
                        ->orWhere('armada_ticket_authorization.employee_id', Auth::user()->id);
                })
            ...
            if ($request->searchBySalesPoint) {
                $armada_ticketing = $armada_ticketing->where('salespoint_id', '=', $request->searchBySalesPoint);
            }
            if ($request->searchByyear) {
                $armada_ticketing = $armada_ticketing->where(DB::raw('year(armada_ticket.created_at)'), '=', $request->searchByyear);
            }
            if ($request->searchByMonth) {
                $armada_ticketing = $armada_ticketing->where(DB::raw('month(armada_ticket.created_at)'), '=', $request->searchByMonth);
            }
            if ($request->searchByArmada) {
                $type = [
                    'pengadaan' => 0,
                    'perpanjangan' => 1,
                    'replace' => 1,
                    ...
                    'end' => 1,
                    'mutasi' => 2,
                    'percepatan_replace' => 4,
                    'percepatan_renewal' => 4,
                    'percepatan_end' => 4
                ];
                if ($type[$request->searchByArmada] == 1) {
                    $armada_ticketing = $armada_ticketing->whereHas('perpanjangan_form', function ($query) use ($request, $type) {
                        if ($request->searchByArmada == 'perpanjangan') {
                            $query->where(DB::raw('lower(perpanjangan_form.form_type)'), 'like', '%' . strtolower($request->searchByArmada) . '%')
                                ->where(DB::raw('perpanjangan_form.is_percepatan'), '=', 0);
                        } else {
                            $query->where(DB::raw('lower(perpanjangan_form.stopsewa_reason)'), 'like', '%' . strtolower($request->searchByArmada) . '%')
                                ->where(DB::raw('perpanjangan_form.is_percepatan'), '=', 0);
                        }
                    });
                } elseif ($type[$request->searchByArmada] == 4) {
                    $armada_ticketing = $armada_ticketing->whereHas('perpanjangan_form', function ($query) use ($request, $type) {
                        if ($request->searchByArmada == 'percepatan_renewal') {
                            $query->where(DB::raw('perpanjangan_form.is_percepatan'), '=', 1)
                                ->where(DB::raw('lower(perpanjangan_form.stopsewa_reason)'), 'like', '%' . 'renewal' . '%');
                        } elseif ($request->searchByArmada == 'percepatan_replace') {
                            $query->where(DB::raw('perpanjangan_form.is_percepatan'), '=', 1)
                                ->where(DB::raw('lower(perpanjangan_form.stopsewa_reason)'), 'like', '%' . 'replace' . '%');
                        } elseif ($request->searchByArmada == 'percepatan_end') {
                            $query->where(DB::raw('perpanjangan_form.is_percepatan'), '=', 1)
                                ->where(DB::raw('lower(perpanjangan_form.stopsewa_reason)'), 'like', '%' . 'end' . '%');
                        }
                    });
                }

                $armada_ticketing = $armada_ticketing->where('armada_ticket.ticketing_type', '=', $type[$request->searchByArmada]);
            }
            $armada_ticketing = $armada_ticketing->get();

            $armada_ticketing = $armada_ticketing->filter(function ($item) use ($request) {
                if ($request->status == -1) {
                    if (in_array($item->status, [-1, 6])) {
                        return true;
                    } else {
                        return false;
                    }
            ...
            foreach ($tickets_paginate as $ticket) {
                ...
                array_push($array, $created_by_employee);
                array_push($array, $ticket->salespoint->name);
                if ($ticket->type() == 'Perpanjangan') {
                    array_push($array, $ticket->type() . "\n( " . $ticket->perpanjangan_form->perpanjangan_length . " Bulan )");
                } else {
                    array_push($array, $ticket->type());
                }
                array_push($array, Carbon::parse($ticket->requirement_date)->translatedFormat('d F Y'));
            ...
            return response()->json([
                "data" => $datas,
                "draw" => $request->draw,
                "recordsFiltered" => $armada_ticketing->count(),
                "recordsTotal" => $armada_ticketing->count(),
            ]);
        }
        if ($request->type == "security") {
            ...
            $security_ticketing =  SecurityTicket::leftJoin('salespoint', 'salespoint.id', '=', 'security_ticket.salespoint_id')
                ...
                ->where(function ($query) use ($employee_access, $search_value) {
                    ...
                    $query->whereIn('security_ticket.salespoint_id', $employee_access)
                        ->orWhere('security_ticket_authorization.employee_id', Auth::user()->id);
                })
                ->where(function ($query) use ($search_value, $ticketing_type_array) {
                    // filter apakan punya akses
                ...
                ->orderBy('security_ticket.created_at', 'asc')
                ->distinct('security_ticket.id');

            if ($request->searchBySalesPoint) {
                $security_ticketing = $security_ticketing->where('salespoint_id', '=', $request->searchBySalesPoint);
            }
            if ($request->searchByyear) {
                $security_ticketing = $security_ticketing->where(DB::raw('year(security_ticket.created_at)'), '=', $request->searchByyear);
            }
            if ($request->searchByMonth) {
                $security_ticketing = $security_ticketing->where(DB::raw('month(security_ticket.created_at)'), '=', $request->searchByMonth);
            }
            if (is_numeric($request->searchBySecurity)) {
                $security_ticketing = $security_ticketing->where('security_ticket.ticketing_type', '=', $request->searchBySecurity);
            }
            $security_ticketing = $security_ticketing->get();

            $security_ticketing = $security_ticketing->filter(function ($item) use ($request) {
                if ($request->status == -1) {
                    if (in_array($item->status, [-1, 6])) {
                        return true;
                    } else {
            ...
            $tickets_paginate = $security_ticketing->skip($request->start)->take($request->length);
            $datas = [];
            $count = 1 + $request->start;
            foreach ($tickets_paginate as $ticket) {
                $array = [];
            ...
            return response()->json([
                "data" => $datas,
                "draw" => $request->draw,
                "recordsFiltered" => $security_ticketing->count(),
                "recordsTotal" => $security_ticketing->count(),
            ]);
        }
    }
    ...
    public function addNewTicket(Request $request)
    {
        ...
        if ($request->ticketing_type == '0') {
            return view('Operational.ticketingdetail', compact('available_salespoints', 'indirect_salespoints', 'budget_category_items', 'vendors', 'filecategories', 'maintenance_budgets', 'ho_budgets', 'fri_authorization'));
        }
        ...
        if ($request->ticketing_type == '3') {
            return view('Operational.Additional.newadditionalticket', compact('available_salespoints', 'vendors'));
        }
        if ($request->ticketing_type == '4') {
            $custom_ticketings = DB::table('custom_ticketing')->get();
            return view('Operational.Additional.newcustomticket', compact('available_salespoints', 'vendors', 'custom_ticketings'));
        }
    ...
    public function addTicket(Request $request)
    {
        try {
            ...
            if ($request->ba_vendor_name != null && $request->ba_vendor_file != null) {
                ...
                $ticket->ba_vendor_filename = "berita_acara_vendor_" . $ticket->code . '.' . $ext;
            ...
            foreach ($allfiles as $afiles) {
                ...
                $salespointname = str_replace(' ', '_', $ticket->salespoint->name);
                $name = $tfile->file_completement->filename . '_' . $ticket->code . '.' . $ext;
                ...
                foreach ($newitems as $key => $item) {
                    ...
                    if (isset($item["attachments"])) {
                        foreach ($item["attachments"] as $attachment) {
                            ...
                            $newAttachment->name = $filename . '_' . $ticket->code . '.' . $ext;
                    ...
                    if (isset($item['files'])) {
                        foreach ($item["files"] as $filereq) {
                            ...
                            $name = $filecompletement->filename . '_' . $ticket->code . '.' . $ext;
        ...
        try {
            ...
            if ($ticket->custom_settings != null) {
                ...
                if ($lpbfile && $invoicefile) {
                    ...
                    $path                           = 'attachments/ticketing/barangjasa/' . $code . '/item' . $ticket_item->id . '/LPB_' . $code . '.' . $lpb_ext;
                    $info                           = pathinfo($path);
                    $lpb_path                       = $lpbfile->storeAs($info['dirname'], $info['basename'], 'public');

                    $path                           = 'attachments/ticketing/barangjasa/' . $code . '/item' . $ticket_item->id . '/INVOICE_' . $code . '.' . $invoice_ext;
                    $info                           = pathinfo($path);
    ...
    public function uploadMissingFile(Request $request)
    {
        try {
            ...
            $path                                = 'attachments/ticketing/barangjasa/' . $code . '/File_kekurangan_berkas_' . $code . '.' . $missingfile_ext;
            $info                                = pathinfo($path);
    ...
    public function revisionMissingFile(Request $request)
    {
        try {
            ...
            $path                                 = 'attachments/ticketing/barangjasa/' . $code . '/File_kekurangan_berkas_' .  $code . '.' . $revisionmissingfile_ext;
            $info                                 = pathinfo($path);
    ...
    public function uploadFileLegal(Request $request, $id)
    {
        try {
            ...
            $path                       = 'attachments/ticketing/barangjasa/' . $code . '/COP'  . '/File_Perjanjian_COP_' . $code . '.' . $perjanjian_ext;
            $info                       = pathinfo($path);
            $perjanjian_path            = $perjanjian_file->storeAs($info['dirname'], $info['basename'], 'public');

            $path                       = 'attachments/ticketing/barangjasa/' . $code . '/COP'  . '/File_TOR_COP_' . $code . '.' . $tor_ext;
            $info                       = pathinfo($path);
            $tor_path                   = $tor_file->storeAs($info['dirname'], $info['basename'], 'public');

            $path                       = 'attachments/ticketing/barangjasa/' . $code . '/COP'  . '/File_SPH_COP_' . $code . '.' . $sph_ext;
            $info                       = pathinfo($path);
    ...
    public function reUploadUserAgreementFileCop(Request $request, $id)
    {
        try {
            ...
            $path                       = 'attachments/ticketing/barangjasa/' . $code . '/COP'  . '/File_Perjanjian_User_COP_' . $code . '.' . $perjanjian_ext;
            $info                       = pathinfo($path);
            $perjanjian_path            = $perjanjian_file->storeAs($info['dirname'], $info['basename'], 'public');
    ...
    public function reUploadBastkFileCop(Request $request, $id)
    {
        try {
            ...
            $path                       = 'attachments/ticketing/barangjasa/' . $code . '/COP'  . '/BASTK_COP_' . $code . '.' . $bastk_ext;
            $info                       = pathinfo($path);
    ...
    public function uploadEvidanceRransferOverPlatform(Request $request, $id)
    {
        try {
            ...
            $path                       = 'attachments/ticketing/barangjasa/' . $code . '/COP'  . '/Over_Plafon_COP_' . $code . '.' . $over_plafon_ext;
            $info                       = pathinfo($path);
    ...
    public function revisionOverPlafonTicket(Request $request, $id)
    {
        try {
            ...
            $path                       = 'attachments/ticketing/barangjasa/' . $code . '/COP'  . '/Over_Plafon_COP_' . $code;
            File::delete($path);
