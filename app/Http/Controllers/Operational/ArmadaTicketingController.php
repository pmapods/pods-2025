...
class ArmadaTicketingController extends Controller
{
            ...
            if ($newTicket->ticketing_type == 0) {
                ...
                if ($request->isNiaga == true || $request->authorSelect == "pr_manual") {
                    if ($request->hasFile('upload_ba_armada_new')) {
                        ...
                        $path                           = '/attachments/ticketing/armada/' . $code . '/new_armada/BA_NEW_ARMADA_' . $code . '.' . $file_ba_new_armada_ext;
                        $info                           = pathinfo($path);
    ...
    public function addPerpanjanganForm(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('upload_ba_renewal')){
                ...
                $path                           = '/attachments/ticketing/armada/' . $armadaticket->code . '/renewal/BA_RENEWAL_' . $armadaticket->code . '.' . $file_ba_renewal_ext;
                $info                           = pathinfo($path);
    ...
    public function addMutasiForm(Request $request)
    {
        ...
        try {
            ...
            do {
                ...
                $count++;
                $checkMutasiForm = MutasiForm::where('armada_ticket_id', $request->armada_ticket_id)->first();
            ...
            if ($flag == false) {
                $form                       = MutasiForm::where('armada_ticket_id', $request->armada_ticket_id)->first();
            }
            else {
                $form                       = new MutasiForm;
            }
            ...
            if ($request->hasFile('upload_ba_mutasi')) {
                ...
                $path                           = '/attachments/ticketing/armada/' . str_replace('/', '-', $code) . '/mutasi/BA_MUTASI_' . str_replace('/', '-', $code) . '.' . $file_ba_mutasi_ext;
                $info                           = pathinfo($path);
            ...
            return back()->with('error', 'Gagal membuat form mutasi: ' . $errorMessage);
        }
    }
    ...
    public function approveMutasiForm(Request $request, $return_data_type = 'view')
    {
        try {
            ...
            $cekarmadaticket = ArmadaTicket::find($mutasi_form->armada_ticket_id);
            if ($cekarmadaticket->ticketing_type != 4) {
                $newrequest = new Request;
                $newrequest->replace([
                    'type' => "armada",
                    'block_type' => "block_mutasi",
                    'ticket_code' => $cekarmadaticket->code,
                ]);
                $response = app('App\Http\Controllers\Operational\TicketingBlockController')->checkTicketingAvailable($newrequest);
                $responseData = $response->getData();
                if ($responseData->error) {
    ...
    public function uploadBASTK(Request $request)
    {
        try {
            ...
            $name = "BASTK_" . $armadaticket->code . '.' . $ext;
            ...
            if (in_array($armadaticket->type(), ['Replace', 'Renewal', 'End Kontrak', 'Percepatan Renewal', 'Percepatan Replace'])) {
                $po = Po::where('no_po_sap', $armadaticket->po_reference_number)->first();
                $pomanual = PoManual::where('po_number', $armadaticket->po_reference_number)->first();
            ...
            if (in_array($armadaticket->type(), ['Pengadaan', 'Replace', 'Renewal', 'Percepatan Renewal', 'Percepatan Replace'])) {
    ...
    public function uploadOldBASTK(Request $request, $id)
    {
        try {
            ...
            $path                       = 'attachments/ticketing/armada/' . $code . '/' . 'BASTK_OLD_' . $code . '.' . $bastk_ext;
            $info                       = pathinfo($path);
    ...
    public function uploadBASTKGT(Request $request)
    {
        try {
            ...
            $salespointname = str_replace(' ', '_', $armadaticket->salespoint->name);
            $ext = pathinfo($request->file('bastk_file')->getClientOriginalName(), PATHINFO_EXTENSION);
            $name = "BASTK_GT_" . $armadaticket->code . '.' . $ext;
            $path = "/attachments/ticketing/armada/" . $armadaticket->code . '/' . $name;
            $file = pathinfo($path);
            $path = $request->file('bastk_file')->storeAs($file['dirname'], $file['basename'], 'public');
